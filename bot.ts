...
(() => {
	const { CHANNEL_TO_LISTEN, CHANNEL_LANG, PCG_USER, TWITCH_OAUTH_TOKEN } = require('./settings.json')
	...
	const channels = [CHANNEL_TO_LISTEN as string];
	const pokebot = PCG_USER as string;
	const client = new tmi.Client({
		options: { debug: false },
		identity: {
			username: 'anyone',
			password: TWITCH_OAUTH_TOKEN as string
		},
		channels: [...channels],
	});
	client.connect().then(() => {
